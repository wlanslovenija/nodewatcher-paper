...
Very early on in its development, a plugin system has been developed and hundreds of community-made plugins have been made since then, many available through their Trac Hacks repository.~\cite{TracHacks_2004}
We were inspired by it and decided to do something similar for managing the community networks.
We build the \nodewatcher{} using Django framework~\cite{django_2005} and not a custom framework to leverage standard Django packages and modularity.
We can reuse package repositories and tools Django ecosystem provides.
For example, Django Packages~\cite{DjangoPackages_2010} repository with more than 2600 packages available.

In 2014 interoperability efforts between community networks have been revived~\cite{interop_2010}.
Quite some active communities starting discussing and comparing data schemas used for configuration and monitoring.
During this ongoing process we have been again evaluating schemas used by other communities and resulted compromises and merged schemas.
Again and again we have been reaffirmed that we achieved our design goal because we are discovering that \nodewatcher{} can support all of proposed schemas with little effort.
Allowing it to be used in any of participating community networks, or even to migrate between used data schemas.
