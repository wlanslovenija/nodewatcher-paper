...
\todo[inline]{Why it comes to differences, various reasons, but again, network is deployed and maintained by community, so this brings a special set of challenges, for example, one cannot provision all devices automatically, but people keep the control of their devices. They experiment. Change things. Some keep old versions, do not upgrade. Maybe the example of compiler. Warnings until it is good. A way to configure the node from scratch.}
...
\todo[inline]{I think we should also talk how we are storing data in each granularity collection in MongoDB. So the thing about \_id manipulation. Just so that it is documented so that we can reference other people to this when they will want to learn more about datastream. Or should we simply leave this to the datastream documentation?}
\todo[inline]{We should cite datastream and make a link to its website.}
...
\todo[inline]{I think we should expand on granularities. I think this idea that we do not just compute average values when downsampling, but also other statistical moments, max, min, standard deviation, which we can then use to visualize data much better, is an important on. We can also move Figure~\ref{fig:interactive-visualization} here and describe it visually. We should also explain that derived values are also downsampled. So this two things work together. So not sure about the order in which we should present this.}
