...
\begin{itemize}
    \item The schema items are class-based, which means they can be extended later on by other modules (in the example \texttt{VIFConfig} extends a more generic \texttt{InterfaceConfig} which may be provided by another module).
...
For example, topology information may be updated more quickly as it is readily available from the OLSR routing protocol and does not require polling of all the nodes.
