...
Using these device descriptors and the platform-independent configuration provided in the first stage, nodewatcher is able to generate device-specific configuration together with a firmware image which may be flashed directly onto the target device.
...
Bundling firmware together with node-specific configuration is a novel way of provisioning community networks that is currently not used in any other community network.

The underlying firmware build system has been built in such a way that it is extensible to different operating systems and platforms (see Figure~\ref{fig:firmware-build-system} for an overview of the whole build pipeline).
