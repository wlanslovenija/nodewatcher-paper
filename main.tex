...
In order to get a complete overview of existing solutions, developed by community networks, we have created a table initially filled with several solutions known to us, and distributed it around to different communities to fill in the rest.
Then we asked them questions and studied the source code (where it is available) regarding the feature sets that these applications have.
The results of this survey are presented in Table~\ref{tbl:comparision}.
...
In the next paragraphs, we describe some of the solutions presented in the table.
...
In addition to just configuration, a firmware image, suitable for the target device, may also be generated (Section~\ref{sec:firmware-generator}) in order to ease deployment.
