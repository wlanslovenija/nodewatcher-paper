...
\newcommand{\git}{G}
\begin{table*}[t!]
\caption{\label{tbl:comparision}Feature comparison of existing node database systems. In the table we show the developer group (DEV) of a project, if it is actively developed (A) and when it was first officially mentioned and deployed (Y). In terms of project functionalities we list network monitoring (NM), configuration generation (CFG), firmware generator (FW), application program interface (API), resource allocation (RA), node authentication (NA), link planning (LP), topology visualization (TP), and map visualization (MP). We also show which projects can be easily extended in terms of user interface (UI), application program interface (API), network monitoring (NM), support of platforms (PL), schema (SC), and routing protocols (RP). In terms of developer accessibility we highlight the code repository used (RCS), documentation status (DOC), programming language (LAN), web framework (WF), and code license (LIC).}
...
The management cycle starts with the \textit{configuration stage} (Figure~\ref{fig:device-mgmt-cycle}, step a) where devices are configured to be used at a specific location in the network.
At this stage, device configuration is still platform-independent and does not depend on the hardware that will be later used to deploy the device (Section~\ref{sec:platform-independent-configuration}). This configuration can be adjusted using a dynamically adjusted web interface (Section~\ref{sec:form-generation}). Finally, the configuration is verified taking into account limited resources of the network (Section~\ref{sec:resource-allocation}). The second stage is the \textit{generation stage} (Figure~\ref{fig:device-mgmt-cycle}, step b), where platform-independent configuration is transformed to device- and platform-specific configuration and a firmware image for the target device may also be generated (Section~\ref{sec:firmware-generator}).
Then, in the \textit{deployment stage} (Figure~\ref{fig:device-mgmt-cycle}, steps c and d), the firmware is applied to the target device and the device is deployed in the field.
...
In the \textit{monitoring stage} (Figure~\ref{fig:device-mgmt-cycle}, step e), the device is actively validated for correct operation in context of the whole network. This 
