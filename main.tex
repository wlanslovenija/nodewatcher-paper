...
None of the existing solutions support generating functional and pre-configured firmware images that can be directly flashed to devices, which reduces the administration burden.
...
It is a small C application with a minimal core which is able to periodically request the loaded modules to provide their state updates which are then compiled into the current node status and exported in a JSON form to be served via HTTP.
...
As can be seen from this description, the monitoring agent is similar in design to other parts of nodewatcher, with a focus on modularity.
...
A single version for all modules is not enough as when modules are developed by different developers, possibly from different community networks and independent schema evolution is required.
