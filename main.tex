...
The modularity makes extending the application easier, but in contrast to \nodewatcher{}, there is no common approach to managing schema extensions and interfaces between modules are still tightly coupled.\todo[inline]{This reads strange. How can interfaces be coupled? Modules are coupled, no?}
...
The automatically generated forms may be customized by the module developers where needed, but even defaults are immediately usable for simple schema items.\todo[inline]{Should we also mention automatically generated REST endpoints?}
...
\todo[inline]{Agent is only one implementation. Otherwise a simple HTTP protocol. But we can also extend this to other protocols which nodes can support, like SNMP. From this we can then nicely transition to modular backend.}
...
The monitoring pipeline may generate large amounts of time-series data during its operation (for example \wlanslovenija{} has accumulated over 200 GiB of data in the last few years, storing everything from network diagnostics to external sensor data).\todo[inline]{Let's be precise here. In how many years?}
...
To illustrate why this can be a problem, imagine a simple 8-bit unsigned integer counter (its size is known to the monitoring platform), sampled every 10 seconds (see Figure~\ref{fig:datastream-counter-reset}).\todo[inline]{I would add also reset stream derivation to show that derivation can be chained. Also, reset stream does not really have 0 when there is no reset?}
...
\todo[inline]{I think we should expand on granularities. I think this idea that we do not just compute average values when downsampling, but also other statistical moments, max, min, standard deviation, which we can then use to visualize data much better, is an important on. We can also move Figure~\ref{fig:interactive-visualization} here and describe it visually.}

\subsection{Modular user interface}

\todo[inline]{Extendable menus and templates. Server-side templates. REST APIs for all data + JavaScript. Modular, interchangeable, exportable. Community can prototype faster new solutions over existing base through or modules for the platform, or just using REST API. We already mentioned extendable forms (and possibly REST), reference that.}
