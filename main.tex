...
For example, there may be a schema item called \texttt{InfoCfg} in the base schema, enabling users to configure a \texttt{name} for a node (see Figure~\ref{fig:registry-schema-example}).\todo[inline]{Why a different example? Could not this be done through same example as in Figure~\ref{fig:schema-module-wifi}, just an entity relationship diagram? Also a concrete example than a dummy example is better.}
...
In traditional computer networks, especially of such scale, provisioning of network devices would be done automatically and centrally. There would be little reason to assume misconfiguration of nodes. Most issues with operation would be because of software or hardware bugs, hardware failure, or human error from operations control. In any case, the misconfiguration would be easy to address: simply provision the node again. On the other hand, most community networks see decentralized nature of their networks an important aspect of their networks and do not want a centralized and automatic control of nodes. Nodes are often maintained by individuals who might use tools like \nodewatcher{} to streamline the process of maintenance, but they still want to retain the sole control of and access to the device. This can lead to potentially many issues, from having devices with old and potentially obsolete firmware versions in the network, to experimentation going berserk, to simply misconfigured nodes. Such nature of community networks has to be taken in the account and \nodewatcher{} helps detect any issues and guide maintainers towards resolving them. They can use provided firmware image, or can be guided through detected issues and suggested steps to resolve them manually. For example, in \wlanslovenija{} network we had cases where maintainers connected to the network vanilla OpenWrt installed devices, \nodewatcher{} detected invalid or missing configuration, provided instructions to resolve them, and maintainers manually vetted and followed them one by one until the node was brought into the compliance with the rest of the network. Such approach is of course time-consuming, but it is an important part of community networks spirit to be able to retain complete control over the node and all aspects of its operation and configuration.
...
\todo[inline]{We explained the extensible configuration schema in XY section. But how we are storing monitored data? Sensors and all this? We have this in two layers. We store latests data into a registry monitoring schema. So that it is available to modules though a relational database. But then we also store all historic data into a time-series data storage. We should have here a diagram, how we have extensible configuration, monitoring, and then from monitoring historical data.}
...
\todo[inline]{We explained ways to get data into the platform. But there are also needs to get data out. In general we have three ways: Static reading, REST API, historic/time-series data reading through datastream. All of that is modular. Similarly to how we are generating automatically forms in XY section, we can also generate APIs and datastreams, and static pages. Forms are just a special case of this. More: extendable menus and templates. Server-side templates. Automatically generated REST endpoints/APIs for all data + JavaScript. Modular, interchangeable, exportable. Community can prototype faster new solutions over existing base through or modules for the platform, or just using REST API. We already mentioned extendable forms (and possibly REST), reference that. Not forget about partials.}
...
\todo[inline]{Faster measurements. Parallel. Distributed load. Multiple/various cycles at various intervals so that some cheap things can be more frequently measured, other who make more load on the network or server, less often. All stored forever so that we can learn, analyze, and share. Open data. Events, alarms, various types, more customizable. Notifications. We have both configuration data and measured data and we can compare.}

\subsection{Interactivity}

The old version used a round-robin database for time series data storage.
\todo[inline]{Compare RRD to the new system, processing time delays, static vs. interactive visualization (see Figure~\ref{fig:interactive-visualization}).}

\todo[inline]{Interactive visualisations. Interactive forms. All of this modular. And with adaptive interfaces for various levels of users (beginners, experts; more or less permissions). Roles, permissions for various types of networks, some are more open than others about who can control the nodes. We can support of all that through modularity.}

\subsection{Modularity and interoperability}

\todo[inline]{Trac as an example of a modular system for managing open source projects. Success of Trac Hacks (some numbers of packages). We are doing something similar for managing the community networks. We are using Django, not custom framework, modules are standard Django packages, we can reuse package repositories and tools Django ecosystem provides. Like Django Packages (some numbers of packages) website, which is similar to Trac Hacks. Together with Django, also Python and their repository (some numbers of packages).}

\todo[inline]{Revived interoperability efforts~\cite{interop_2010} in the past half of year, communities discuss various data schemas, etc. We are taking part and discovering that nodewatcher can support all of that with little efforts. Victory.}

\section{Conclusion and Future Work}
\label{sec:conclusion}

\todo[inline]{We conclude that we achieved our goal of creating a modular platform with guiding thought of being useful also to other communities (while other software was often made for only a particular community), by being modular and addressing all other shortcomings we have realized through all this years. Now the real work start, working with other communities. Do we menton that we are organizing Battlemesh which is a great place to launch this?}

\todo[inline]{Radio planning features. Multiple routing protocols. But mostly in the future working with other communities to start developing customized modules. So we do not want to be developing everything, now others can start picking up.}

\todo[inline]{Explain how our data can be used in research to learn more about community collaboration and real-world networks. Example of such research is CONFINE. We are doing something similar for our KORUZA project where we will be using nodewatcher to monitor deployments all around the world.}
...
\todo[inline]{I think we should also thank the Slovenian community, and wider global community networks community. Do we want to thank any particular individuals or organisations?}
