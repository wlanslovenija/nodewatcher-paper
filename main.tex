...
It is implemented using a popular Django web framework ORM \cite{django_2005}.
...
\todo[inline]{We should talk here also about possibility of generating different forms for same schema, making variations of forms which do not expose all features and not confuse novice users. Defaults are then applied in the background. Also, various levels of users might have different level of what can see and change.}
...
This is necessary especially in community networks where coordination between volunteers is lack.
Nodes may be removed from the database, but not yet permanently from the network and may actually reappear at a later time, causing routing conflicts.
...
\todo[inline]{- we should show how an actual deployment looks like in \wlanslovenija{} (a nice schematic figure)
